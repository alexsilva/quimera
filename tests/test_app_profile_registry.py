...
class TestProfileRegistryInjection:
    ...
    def test_default_no_registry_uses_profiles_module(self, tmp_path, mock_gate):
        """Sem registry injetado, usa profiles.get / profiles.all_profiles (fallback)."""
        app = QuimeraApp(tmp_path, input_gate_factory=mock_gate)
    ...
    def test_injected_empty_registry_returns_no_profiles(self, tmp_path, empty_registry, mock_gate):
        """Verifica que registry vazio injetado retorna lista vazia e None para consultas."""
        app = QuimeraApp(tmp_path, profile_registry=empty_registry, input_gate_factory=mock_gate)
        available = app.get_available_profiles()
        assert available == []
        profile = app.get_agent_profile("codex")
        assert profile is None

    def test_injected_registry_with_profile_returns_it(self, tmp_path, populated_registry, sample_profile, mock_gate):
        """Verifica que registry populado retorna os profiles corretos."""
        app = QuimeraApp(tmp_path, profile_registry=populated_registry, input_gate_factory=mock_gate)
        available = app.get_available_profiles()
        assert available == [sample_profile]
        profile = app.get_agent_profile("test-agent")
        assert profile is sample_profile

    def test_injected_registry_unknown_agent_returns_none(self, tmp_path, populated_registry, mock_gate):
        """Verifica que agente inexistente no registry retorna None."""
        app = QuimeraApp(tmp_path, profile_registry=populated_registry, input_gate_factory=mock_gate)
        profile = app.get_agent_profile("unknown-agent")
        assert profile is None

    def test_default_registry_isolation_from_injected(self, tmp_path, populated_registry, mock_gate):
        """Verifica que dois apps com registries diferentes não compartilham profiles."""
        app1 = QuimeraApp(tmp_path, profile_registry=populated_registry, input_gate_factory=mock_gate)
        app2 = QuimeraApp(tmp_path, input_gate_factory=mock_gate)  # Sem registry -> usa modulo profiles global
    ...
    def test_get_agent_profile_normalizes_name_with_registry(self, tmp_path, populated_registry, sample_profile, mock_gate):
    ...
    def test_get_agent_profile_empty_name_returns_none(self, tmp_path, empty_registry, mock_gate):
    ...
    def test_available_commands_uses_only_selected_agents_prefixes(self, tmp_path, mock_gate):
        ...
        app = QuimeraApp(tmp_path, profile_registry=registry, agents=["codex"], input_gate_factory=mock_gate)
