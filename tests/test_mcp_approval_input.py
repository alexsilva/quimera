...
def test_input_broker_line_select_does_not_emit_anchored_question():
    """Broker fallback must not duplicate selection text already in anchored card."""
    emitted = []
    window_metadata = []

    class FakeDeck:
        """Deck stub with one active stream owner."""
    ...
    class FakeRenderer:
        """Renderer stub exposing selection windows and active deck."""
        ...
        def selection_window(self, **kwargs):
            """Capture selection metadata for assertions."""
            window_metadata.append(kwargs)
            yield

    gate = MagicMock()
    gate.read_input_in_terminal.return_value = "1"
    broker = InputBroker(renderer=FakeRenderer(), input_gate=gate)
    broker._emit = emitted.append

    result = broker._line_select(
        "Qual ação tomar?",
        ["Centralizar", "Manter duplicação"],
        deadline=time.monotonic() + 10,
        agent="claude",
    )

    assert result == (0, "Centralizar")
    assert emitted == []
    assert window_metadata[0]["metadata"]["options"] == [
        "Centralizar",
        "Manter duplicação",
    ]
    gate.read_input_in_terminal.assert_called_once()
...
def test_input_broker_approval_does_not_emit_anchored_question():
    """Broker approval fallback must not duplicate anchored approval text."""
    emitted = []
    ...
    class FakeRenderer:
        """Renderer stub exposing approval windows and active deck."""
    ...
    request = type(
        ...
        {
            "source": "claude",
            "question": "Executar ação?",
            "timeout": 10,
            "on_approve_all": None,
        },
    )()

    assert broker._handle_approval(request) is True
    assert emitted == []
