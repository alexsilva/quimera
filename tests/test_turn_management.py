...
from quimera.app.worker import ChatWorker, ChatWorkItem
...
class DummyRenderer(RendererBase):
    ...
    def show_message(self, *a, **kw): self.messages.append((a, kw, time.monotonic()))
...
def _make_app(active_agents=None):
    ...
    # Comportamento padrão: primeira chamada retorna resposta simples.
...
class TestTurnCycle(unittest.TestCase):
    ...
    def test_run_keeps_chat_alive_on_input_interrupt_with_queued_prompt(self):
        """Ctrl+C com prompt pendente deve cancelar o trabalho, não encerrar o chat."""
        app = QuimeraApp.__new__(QuimeraApp)
        from quimera.app.runtime_state import AppRuntimeState
        app.runtime_state = AppRuntimeState()
        app.renderer = DummyRenderer()
        ...
        self.assertEqual(process_calls, ["mensagem-ativa", "mensagem-pendente"])
        self.assertEqual(len(read_calls), 4, "run() deveria tratar Ctrl+C e voltar ao input")
        self.assertTrue(app.turn_manager.is_human_turn)
    ...
    def test_run_queues_until_slot_frees_without_using_main_thread(self):
        """Prompt excedente aguarda no executor sem bloquear o loop principal."""
        app = QuimeraApp.__new__(QuimeraApp)
        from quimera.app.runtime_state import AppRuntimeState
        app.runtime_state = AppRuntimeState()
        app.renderer = DummyRenderer()
        ...
        self.assertEqual([mode for _, mode, _ in calls], ["async", "async", "async"])
        self.assertGreaterEqual(elapsed, 0.20)

    def test_completed_response_is_rendered_while_other_prompts_are_running(self):
        """A main thread deve drenar a UI assim que cada resposta termina."""
        app = QuimeraApp.__new__(QuimeraApp)
        from quimera.app.runtime_state import AppRuntimeState
        app.runtime_state = AppRuntimeState()
        app.renderer = DummyRenderer()
        ...
        read_index = [0]
        rendered_with_work_outstanding = [False]

        def mock_read_user_input(prompt, timeout):
            read_index[0] += 1
            if read_index[0] <= 3:
                return f"m{read_index[0]}"
            if app.renderer.messages:
                rendered_with_work_outstanding[0] = (
                    app.runtime_state.get_chat_outstanding_count() > 0
                )
                return CMD_EXIT
            return None

        durations = {"m1": 0.05, "m2": 0.30, "m3": 0.30}

        def observed_process(user):
            time.sleep(durations[user])
            app._ui_event_queue.put(
                RenderEvent(RenderEvent.TEXT, f"resposta {user}", agent=user)
            )
        ...
        with patch("quimera.app.chat_processor.sys.stdin", tty_stdin):
            _materialize_ui_event_handler(app)
            QuimeraApp.run(app)
    ...
    def test_run_enqueues_before_switching_turn_with_worker(self):
        ...
        class ObservedQueue(queue.Queue):
            def put(self, item, block=True, timeout=None):
                if item not in (None,):
                    payload = item.message if isinstance(item, ChatWorkItem) else item
                    order.append(f"put:{payload}")
...
class TestSingleAgentPerTurn(unittest.TestCase):
    ...
    def test_default_mode_only_first_agent_responds(self):
        """Sem prefixo explícito, apenas um agente responde por turno."""
        app = _make_app(active_agents=["claude", "codex"])
        # Roteamento padrão: sem prefixo explícito
        app.parse_routing = Mock(return_value=("claude", "olá", False))
        # Resposta comum, sem envelope de controle.
        app.parse_response = Mock(return_value=("resposta", None, None, None))
    ...
    def test_explicit_prefix_still_single_agent(self):
        """Prefixo explícito garante que apenas um agente responde."""
        app = _make_app(active_agents=["claude", "codex"])
        app.parse_routing = Mock(return_value=("claude", "faz algo", True))
        app.parse_response = Mock(return_value=("resposta", None, None, None))
    ...
    def test_delegation_triggers_secondary_agent(self):
        ...
        responses = [
            ...
            ("resposta claude", "codex", delegation_payload, None),
            # Resposta do codex (delegation_only)
            ("resposta codex", None, None, "abc123"),
            # Síntese do claude
            ("síntese", None, None, None),
        ]
    ...
    def test_self_delegation_is_ignored(self):
        ...
        app.parse_response = Mock(return_value=("resposta claude", "claude", delegation_payload, None))
    ...
    def test_delegation_secondary_can_delegate_to_third_before_synthesis(self):
        ...
        app.parse_response = Mock(side_effect=[
            ("resposta claude", "codex", first_delegation, None),
            ("resposta codex", "opencode-qwen", second_delegation, "h1"),
            ("resposta qwen", None, None, "h2"),
            ("síntese final", None, None, None),
        ])
    ...
    def test_delegation_circular_chain_does_not_loop(self):
        ...
        parsed = iter([
            ("resposta claude", "codex", first_delegation, None),
            ("resposta codex", "claude", circular_delegation, "h1"),
            ("síntese", None, None, None),
        ])
    ...
    def test_delegation_without_body_continues_chain(self):
        ...
        app.parse_response = Mock(side_effect=[
            ("resposta claude", "codex", first_delegation, None),
            (None, "opencode-qwen", second_delegation, "h1"),  # sem body
            ("resposta qwen", None, None, "h2"),
            ("síntese final", None, None, None),
        ])
        app.dispatch_services.delegate = Mock(side_effect=["r1", "r2", "r3", "r4"])
    ...
    def test_delegation_to_unknown_agent_is_ignored(self):
        ...
        app.parse_response = Mock(return_value=("resposta claude", "agente", delegation, None))
    ...
    def test_sequential_delegations_accumulate_all_delegate_responses_for_synthesis(self):
        ...
        app.parse_response = Mock(side_effect=[
            (None, "codex", first_delegation, None),
            ("resposta codex", None, None, "h1"),
            ("resposta qwen", None, None, "h2"),
            ("síntese final", None, None, None),
        ])
        app.dispatch_services.delegate = Mock(side_effect=["r1", "r2", "r3", "r4"])
