...
def test_apply_patch_dry_run_validates_without_writing(tmp_path):
    """dry_run valida o patch sem modificar arquivos."""
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    target = workspace / "demo.txt"
    target.write_text("linha 1\nlinha 2\n", encoding="utf-8")
    ...
    result = tool.apply_patch(
        ToolCall(name="apply_patch", arguments={"patch": patch, "dry_run": True})
    )

    assert result.ok is True
    assert result.content.startswith("[dry-run]")
    assert result.data["dry_run"] is True
    assert result.data["changed_files"] == ["demo.txt"]
    assert target.read_text(encoding="utf-8") == "linha 1\nlinha 2\n"


def test_apply_patch_dry_run_add_file_does_not_create_file(tmp_path):
    """dry_run de Add File valida path e existência sem criar arquivo."""
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    ...
    assert result.ok is True
    assert result.data["changed_files"] == ["novo.txt"]
