...
class TestAppInputServices:
    def test_init(self, mock_app, resolver):
        ...
        srv = AppInputServices(mock_app, resolver)
...
class TestReadUserInput:
    def test_timeout_positive_returns_value(self, mock_app):
        ...
        with patch("quimera.app.inputs.read_user_input_with_timeout") as mock_r:
            ...
            result = read_user_input(mock_app, ">", 30, input_fn=MagicMock())
            assert result == "ok"

    def test_timeout_positive_returns_none(self, mock_app):
        from quimera.app.inputs import read_user_input
        with patch("quimera.app.inputs.read_user_input_with_timeout") as mock_r:
    ...
    def test_timeout_zero_stdin_none(self, mock_app):
        from quimera.app.inputs import read_user_input
        with patch("quimera.app.inputs._stdin", return_value=None):
            result = read_user_input(mock_app, ">", 0, input_fn=MagicMock())
            assert result is None

    def test_timeout_zero_tty_input(self, mock_app):
        from quimera.app.inputs import read_user_input
        mock_stdin = MagicMock()
        mock_stdin.isatty.return_value = True
        mock_input_fn = MagicMock(return_value="line")
        with patch("quimera.app.inputs._stdin", return_value=mock_stdin):
            result = read_user_input(mock_app, ">", 0, input_fn=mock_input_fn)
            assert result == "line"
            assert mock_app._nonblocking_input_status == "idle"
    ...
    def test_timeout_zero_tty_keyboard_interrupt(self, mock_app):
        ...
        with patch("quimera.app.inputs._stdin", return_value=mock_stdin):
            with pytest.raises(KeyboardInterrupt):
                read_user_input(mock_app, ">", 0, input_fn=mock_input_fn)
            assert mock_app._nonblocking_input_status == "idle"

    def test_timeout_zero_non_tty_select_ready(self, mock_app):
        from quimera.app.inputs import read_user_input
        mock_stdin = MagicMock()
    ...
    def test_timeout_zero_non_tty_select_not_ready(self, mock_app):
        from quimera.app.inputs import read_user_input
        mock_stdin = MagicMock()
        mock_stdin.isatty.return_value = False
        with patch("quimera.app.inputs._stdin", return_value=mock_stdin):
    ...
    def test_timeout_zero_non_tty_select_exception(self, mock_app):
        from quimera.app.inputs import read_user_input
        mock_stdin = MagicMock()
        mock_stdin.isatty.return_value = False
        with patch("quimera.app.inputs._stdin", return_value=mock_stdin):
    ...
    def test_timeout_zero_non_tty_eof(self, mock_app):
        from quimera.app.inputs import read_user_input
        mock_stdin = MagicMock()
        mock_stdin.isatty.return_value = False
    ...
    def test_timeout_none_eof(self, mock_app):
        from quimera.app.inputs import read_user_input
        input_fn = MagicMock(side_effect=EOFError)
        with pytest.raises(EOFError):
            read_user_input(mock_app, ">", None, input_fn=input_fn)

    def test_timeout_none_keyboard_interrupt(self, mock_app):
        from quimera.app.inputs import read_user_input
    ...
    def test_timeout_none_normal(self, mock_app):
        from quimera.app.inputs import read_user_input
        input_fn = MagicMock(return_value="hello")
        result = read_user_input(mock_app, ">", None, input_fn=input_fn)
        assert result == "hello"
        assert mock_app._nonblocking_prompt_visible is False

    def test_timeout_zero_stdin_isatty_exception(self, mock_app):
        from quimera.app.inputs import read_user_input
        mock_stdin = MagicMock()
...
class TestReadUserInputNonblockingTty:
    def test_creates_queue_if_none(self, mock_app):
        from quimera.app.inputs import read_user_input_nonblocking_tty
        mock_app._nonblocking_input_queue = None
        with patch("queue.Queue.get_nowait", side_effect=queue.Empty):
            with patch("quimera.app.inputs.start_nonblocking_input_reader"):
                result = read_user_input_nonblocking_tty(mock_app, ">")
                assert result is None

    def test_queue_empty_starts_reader(self, mock_app):
        from quimera.app.inputs import read_user_input_nonblocking_tty
        mock_app._nonblocking_input_queue = queue.Queue()
        with patch("quimera.app.inputs.start_nonblocking_input_reader") as mock_start:
            result = read_user_input_nonblocking_tty(mock_app, ">")
            assert result is None
            mock_start.assert_called_once()

    def test_queue_empty_thread_alive_sleeps(self, mock_app):
        from quimera.app.inputs import read_user_input_nonblocking_tty
        q = queue.Queue()
        mock_app._nonblocking_input_queue = q
        t = threading.Thread(target=lambda: None)
        t.daemon = True
        mock_app._nonblocking_input_thread = t
        with patch("time.sleep"):
            result = read_user_input_nonblocking_tty(mock_app, ">")
            assert result is None

    def test_queue_line(self, mock_app):
        from quimera.app.inputs import read_user_input_nonblocking_tty
        q = queue.Queue()
        q.put(("line", "hello"))
        mock_app._nonblocking_input_queue = q
        ...
        assert result == "hello"
        assert mock_app._nonblocking_input_status == "idle"
        mock_app.system_layer.flush_deferred_messages.assert_called_once()

    def test_queue_interrupt(self, mock_app):
        from quimera.app.inputs import read_user_input_nonblocking_tty
        q = queue.Queue()
        q.put(("interrupt", None))
        mock_app._nonblocking_input_queue = q
        with pytest.raises(KeyboardInterrupt):
            read_user_input_nonblocking_tty(mock_app, ">")

    def test_queue_eof(self, mock_app):
        from quimera.app.inputs import read_user_input_nonblocking_tty
        q = queue.Queue()
        q.put(("eof", None))
        mock_app._nonblocking_input_queue = q
    ...
    def test_queue_error(self, mock_app):
        from quimera.app.inputs import read_user_input_nonblocking_tty
        q = queue.Queue()
        q.put(("error", None))
        mock_app._nonblocking_input_queue = q
...
class TestStartNonblockingInputReader:
    def test_creates_queue_if_none(self, mock_app):
        from quimera.app.inputs import start_nonblocking_input_reader
        mock_app._nonblocking_input_queue = None
        with patch("threading.Thread"):
            start_nonblocking_input_reader(mock_app, ">")
            assert mock_app._nonblocking_input_queue is not None
    ...
    def test_no_editor_found(self, mock_app):
        ...
        with patch("shutil.which", return_value=None):
            with patch.dict("os.environ", {}, clear=True):
                result = read_from_editor(mock_app)
                assert result is None
...
class TestReadFromFile:
    def test_file_not_found(self, mock_app):
        ...
        with patch("pathlib.Path.exists", return_value=False):
            result = read_from_file(mock_app, "/nonexistent")
            assert result is None
            mock_app.renderer.show_error.assert_called_once()
    ...
    def test_file_found(self, mock_app):
        ...
        with patch("pathlib.Path.exists", return_value=True):
            with patch("pathlib.Path.read_text", return_value="hello\n"):
                result = read_from_file(mock_app, "/some/path")
                assert result == "hello"
